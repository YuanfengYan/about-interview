...
## 基础知识
  ...
  1. ✅ http协议，状态码 http1.1和2的区别 为什么可以多路复用，[header压缩怎么压缩](https://segmentfault.com/a/1190000017011816)、[http2和1的改进](https://www.zhihu.com/question/34074946)、[状态码-传送门](./md/http状态码.md) 、 [TCP/IP协议与Http协议的区别](https://www.cnblogs.com/xianlei/p/tcpip_http.html)
...
## 项目中遇到的问题
  ...
  2. ios 中不能使用animation替换image来实现序列帧(存在兼容性问题)。 1、需要用雪碧图（[雪碧图生成工具](https://www.toptal.com/developers/css/sprite-generator)）更改position； 2、js动态替换img
  3. 动态列表过渡效果：在上一个列表上实现渐显覆盖过渡。--》生成两个列表，两个列表通过层级控制显示为当前列表。每次新数据进入，推入另一个列表并设置为当前层级，js控制过渡效果。
  4. ios的打开app的通用链接配置 [参考文档](https://www.jianshu.com/p/ab50bdaec65d)
...
## 其他
  ...
  11. 如何带团队的，参与过什么重大的技术问题。 
        + 如何带团队的 ---- 1、团队技术精进。2、项目需求安排，技术方案选择、项目进度站会。3、代码review。 4、组织团建增加团队凝聚力。 
